...
class VersionControlSystem {
  ...
  VersionControlSystem() {
    ...
    _branches['main'] = Branch(
      name: 'main',
      commitId: initialCommit.id,
    );
  }
  
  Commit commit(String message, {String? author, required PresentationTree tree}) {
    final commit = Commit(
      id: _generateCommitId(),
      message: message,
  ...
  Commit? revert(String commitId) {
    ...
    final revertCommit = Commit(
      ...
      timestamp: DateTime.now(),
      parentIds: _headCommit != null ? [_headCommit!] : [],
      tree: targetCommit.tree, // In real implementation, compute inverse
    );
  ...
  List<Commit> getHistory({String? branchName, int? limit}) {
    final branch = branchName != null ? _branches[branchName] : _branches[_headBranch];
  ...
  PresentationTree _mergeTrees(PresentationTree current, PresentationTree target, PresentationTree? base) {
    // Three-way merge
    ...
    for (final slideId in allSlideIds) {
      ...
      if (currentSlide == null) {
        ...
        // Modified in both
        mergedSlides[slideId] = _mergeSlides(currentSlide, targetSlide, baseSlide);
      }
    }
  ...
  SlideSnapshot _mergeSlides(SlideSnapshot current, SlideSnapshot target, SlideSnapshot? base) {
    ...
    for (final elementId in allElementIds) {
      ...
      if (currentElement == null) {
        ...
        mergedElements[elementId] = targetElement.timestamp.isAfter(currentElement.timestamp)
            ? targetElement
            : currentElement;
      }
    }
    ...
    return SlideSnapshot(
      ...
      background: target.timestamp.isAfter(current.timestamp) ? target.background : current.background,
      timestamp: DateTime.now(),
    );
  }
  ...
  bool _slidesEqual(SlideSnapshot a, SlideSnapshot b) {
    return a.elements.length == b.elements.length &&
        a.elements.keys.every((k) => b.elements.containsKey(k));
  }
  
  String _generateCommitId() {
    final bytes = utf8.encode('${DateTime.now().millisecondsSinceEpoch}_${Random().nextInt(1000000)}');
...
class PresentationTree {
  ...
  factory PresentationTree.fromJson(Map<String, dynamic> json) => PresentationTree(
    slides: (json['slides'] as Map<String, dynamic>).map(
      (k, v) => MapEntry(k, SlideSnapshot.fromJson(v as Map<String, dynamic>)),
    ),
...
class SlideSnapshot {
  ...
  factory SlideSnapshot.fromJson(Map<String, dynamic> json) => SlideSnapshot(
    ...
    elements: (json['elements'] as Map<String, dynamic>).map(
      (k, v) => MapEntry(k, ElementSnapshot.fromJson(v as Map<String, dynamic>)),
    ),
...
class ElementSnapshot {
  ...
  factory ElementSnapshot.fromJson(Map<String, dynamic> json) => ElementSnapshot(
    id: json['id'] as String,
                ...
                return ListTile(
                  ...
                  trailing: PopupMenuButton<String>(
                    ...
                    itemBuilder: (_) => [
                      ...
                      const PopupMenuItem(value: 'compare', child: Text('Compare with previous')),
                    ],
                  ),
                );
              },
