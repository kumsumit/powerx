...
class _AdvancedChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(
      plotArea,
      Paint()..color = style.plotAreaColor,
    );
  ...
  Rect _calculatePlotArea(Size size) {
    ...
    double right = size.width - (showLegend && legendPosition == LegendPosition.right ? 120 : 20);
  ...
  void _drawColumnChart(Canvas canvas, Rect plotArea) {
    final isStacked = type == AdvancedChartType.columnStacked || type == AdvancedChartType.columnStacked100;
    ...
    final barWidth = isStacked ? groupWidth * 0.6 : (groupWidth * 0.8) / seriesCount;
    ...
    for (int cat = 0; cat < categoryCount; cat++) {
      ...
      for (int ser = 0; ser < seriesCount; ser++) {
        ...
        final barHeight = (normalizedValue / (is100 ? 1 : data.maxValue)) * plotArea.height;
        
        final x = isStacked
            ? plotArea.left + cat * groupWidth + (groupWidth - barWidth) / 2
            : plotArea.left + cat * groupWidth + (groupWidth - seriesCount * barWidth) / 2 + ser * barWidth;
        ...
        // Data label
        if (showDataLabels) {
          _drawDataLabel(canvas, x + barWidth / 2, y - 5, value.toStringAsFixed(1));
        }
  ...
  void _drawBarChart(Canvas canvas, Rect plotArea) {
    ...
    final barHeight = isStacked ? groupHeight * 0.6 : (groupHeight * 0.8) / seriesCount;
    ...
    for (int cat = 0; cat < categoryCount; cat++) {
      ...
      for (int ser = 0; ser < seriesCount; ser++) {
        ...
        final y = isStacked
            ...
            : plotArea.top + cat * groupHeight + (groupHeight - seriesCount * barHeight) / 2 + ser * barHeight;
  ...
  void _drawBubbleChart(Canvas canvas, Rect plotArea) {
    ...
    final sizeMax = data.data.map((r) => r.length > 2 ? r[2] : 1.0).reduce((a, b) => a > b ? a : b);
  ...
  void _drawAxes(Canvas canvas, Rect plotArea) {
    ...
    for (int i = 0; i <= steps; i++) {
      ...
      textPainter.layout();
      textPainter.paint(canvas, Offset(plotArea.left - textPainter.width - 5, y - textPainter.height / 2));
    }
    
    // X axis labels
    final labelStep = max(1, data.rowHeaders.length ~/ 10);
    for (int i = 0; i < data.rowHeaders.length; i += labelStep) {
      final x = plotArea.left + (plotArea.width * i / (data.rowHeaders.length - 1));
      ...
      textPainter.layout();
      textPainter.paint(canvas, Offset(x - textPainter.width / 2, plotArea.bottom + 5));
    }
  }
  ...
  void _drawLegend(Canvas canvas, Size size, Rect plotArea) {
    ...
    for (int i = 0; i < legendItems.length; i++) {
      ...
      canvas.drawRect(
        ...
        Paint()..color = data.seriesColors.length > i
            ? data.seriesColors[i]
            : _defaultColor(i),
      );
